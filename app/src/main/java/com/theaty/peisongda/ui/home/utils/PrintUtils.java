...
//            Toast.makeText(, "发送失败！", Toast.LENGTH_SHORT).show();
            e.printStackTrace();
        }
    }
     ...
     * @param command 格式指令
     */
    public static void selectCommand(byte[] command, boolean isfirst) {
        try {
            outputStream.write(command);
            outputStream.flush();
        } catch (Exception e) {
//            Toast.makeText(this.content, "发送失败！", Toast.LENGTH_SHORT).show();
            e.printStackTrace();
        }
    }
     ...
     * 切纸
     */
    public static final byte[] QIEZHI = {0x1d, 0x56, 0x31};

    /**
     * 设置默认行间距
     */
    public static final byte[] LINE_SPACING_DEFAULT = {0x1b, 0x33, 0x14};
