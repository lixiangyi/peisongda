...
public class TagLayout extends ViewGroup {
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            TypedArray mTypedArray = context.obtainStyledAttributes(attrs, R.styleable.tags_view);

            mTextColor = mTypedArray.getColor(R.styleable.tags_view_labelTextColor, Color.BLACK);
            mTextSize = mTypedArray.getDimension(R.styleable.tags_view_labelTextSize, 42.0f);
            mTextPaddingLeft = mTypedArray.getDimensionPixelOffset(
                    R.styleable.tags_view_labelTextPaddingLeft, 0);
            mTextPaddingTop = mTypedArray.getDimensionPixelOffset(
                    R.styleable.tags_view_labelTextPaddingTop, 0);
            mTextPaddingRight = mTypedArray.getDimensionPixelOffset(
                    R.styleable.tags_view_labelTextPaddingRight, 0);
            mTextPaddingBottom = mTypedArray.getDimensionPixelOffset(
                    R.styleable.tags_view_labelTextPaddingBottom, 0);
            mLineMargin = mTypedArray.getDimensionPixelOffset(R.styleable.tags_view_labellineMargin, 0);
            mWordMargin = mTypedArray.getDimensionPixelOffset(R.styleable.tags_view_wordMargin, 0);
            mLabelBgRes = mTypedArray.getResourceId(R.styleable.tags_view_labelBackground, -1);
            mTypedArray.recycle();
        }
    }
