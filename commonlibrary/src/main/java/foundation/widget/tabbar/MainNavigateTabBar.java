package foundation.widget.tabbar;
...
import foundation.util.ThemeUtils;
import foundation.widget.BadgeView;
...
//        holder.tag = tabParam.title;
		holder.tag = String.valueOf(default_tag++);
		...
		holder.badgeView = new BadgeView(getContext());
		if (TextUtils.isEmpty(tabParam.title)) {
...
//		transaction.commit();
	...
	/**
	 * 隐藏消息数量提示 * @param index 底部导航的位置 * @param count 消息数量 count >0 显示
	 */
	...
	/**
	 * 隐藏消息数量提示 * @param index 底部导航的位置 * @param count 消息数量 count = 0 消失
	 */
